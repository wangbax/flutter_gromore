...
public class FeedAdUtils {
  ...
  public static View getFeedAdFromFeedInfo(TTFeedAd feedAd, Activity activity, View convertView,
  ...
  private static View getVerticalAdView(ViewGroup parent, View convertView, TTFeedAd ad,
      Activity activity, TTNativeAd.AdInteractionListener feedInteractionListener) {
    VerticalAdViewHolder adViewHolder = null;
    MediationViewBinder viewBinder = null;
    if (convertView == null) {
      convertView = LayoutInflater.from(activity)
          .inflate(R.layout.mediation_listitem_ad_vertical_pic, parent, false);
...
//      adViewHolder.version_name = convertView.findViewById(R.id.version_name);
    ...
    if (ad.getImageList() != null && ad.getImageList().size() > 0) {
      Glide.with(activity).load(ad.getImageList().get(0).getImageUrl())
  ...
  private static View getVideoView(ViewGroup parent, View convertView, TTFeedAd ad,
      Activity activity, TTNativeAd.AdInteractionListener feedInteractionListener) {
    VideoAdViewHolder adViewHolder = null;
    MediationViewBinder viewBinder = null;
    if (convertView == null) {
      convertView = LayoutInflater.from(activity)
          .inflate(R.layout.mediation_listitem_ad_large_video, parent, false);
...
//      adViewHolder.version_name = convertView.findViewById(R.id.version_name);
  ...
  private static View getLargeAdView(ViewGroup parent, View convertView, TTFeedAd ad,
      Activity activity, TTNativeAd.AdInteractionListener feedInteractionListener) {
    LargeAdViewHolder adViewHolder = null;
    MediationViewBinder viewBinder = null;
    if (convertView == null) {
      convertView = LayoutInflater.from(activity)
          .inflate(R.layout.mediation_listitem_ad_large_pic, parent, false);
...
//      adViewHolder.version_name = convertView.findViewById(R.id.version_name);
  ...
  private static View getGroupAdView(ViewGroup parent, View convertView, TTFeedAd ad,
      Activity activity, TTNativeAd.AdInteractionListener feedInteractionListener) {
    GroupAdViewHolder adViewHolder = null;
    MediationViewBinder viewBinder = null;
    if (convertView == null) {
      convertView = LayoutInflater.from(activity)
          .inflate(R.layout.mediation_listitem_ad_group_pic, parent, false);
...
//      adViewHolder.version_name = convertView.findViewById(R.id.version_name);
  ...
  private static View getSmallAdView(ViewGroup parent, View convertView, TTFeedAd ad,
      Activity activity, TTNativeAd.AdInteractionListener feedInteractionListener) {
    SmallAdViewHolder adViewHolder = null;
    MediationViewBinder viewBinder = null;
    if (convertView == null) {
      convertView = LayoutInflater.from(activity)
          .inflate(R.layout.mediation_listitem_ad_small_pic, null, false);
...
//      adViewHolder.version_name = convertView.findViewById(R.id.version_name);
  ...
  private static void bindData(View convertView, AdViewHolder adViewHolder, TTFeedAd ad,
    ...
    // 重要! 这个涉及到广告计费，必须正确调用。
    ad.registerViewForInteraction(activity, (ViewGroup) convertView, clickViewList,
        creativeViewList, null, feedInteractionListener, viewBinder);
    ...
    switch (ad.getInteractionType()) {
      ...
      case TTAdConstant.INTERACTION_TYPE_DOWNLOAD:
        ...
        adCreativeButton.setText(
            TextUtils.isEmpty(ad.getButtonText()) ? "立即下载" : ad.getButtonText());
        break;
      ...
      case TTAdConstant.INTERACTION_TYPE_BROWSER:
        adCreativeButton.setVisibility(View.VISIBLE);
        adCreativeButton.setText(
            TextUtils.isEmpty(ad.getButtonText()) ? "查看详情" : ad.getButtonText());
        break;
      default:
  ...
  private static void setDownLoadAppInfo(TTFeedAd ttNativeAd, AdViewHolder adViewHolder) {
    ...
    if (ttNativeAd == null || ttNativeAd.getComplianceInfo() == null) {
      ...
      String infoStr = "应用名称：" + appInfo.getAppName() +
          "开发者：" + appInfo.getDeveloperName() +
          "隐私协议：" + appInfo.getPrivacyUrl() +
          "版本号：" + appInfo.getAppVersion() +
          "权限内容:" + getPermissionsContent(appInfo.getPermissionsMap());
      adViewHolder.app_info.setText(infoStr);
...
//      adViewHolder.permissions_content.setText(
//          "权限内容:" + getPermissionsContent(appInfo.getPermissionsMap()));
    }
  }
